(ns com.time-tracker
  (:require [com.biffweb :as biff]
            [com.time-tracker.feat.app :as app]
            [com.time-tracker.feat.auth :as auth]
            [com.time-tracker.feat.home :as home]
            [com.time-tracker.feat.worker :as worker]
            [com.time-tracker.schema :refer [malli-opts]]
...
(defn generate-assets! [sys]
  (when (:com.time-tracker/enable-web sys)
...
(defn on-save [sys]
  (biff/eval-files! sys)
  (generate-assets! sys)
  (test/run-all-tests #"com.time-tracker.test.*"))

(defn start []
  (biff/start-system
    {:com.time-tracker/chat-clients (atom #{})
     ...
     :biff/components [biff/use-config
                       ...
                       (biff/use-when
                         :com.time-tracker/enable-web
                         biff/use-outer-default-middleware
                         biff/use-jetty)
                       (biff/use-when
                         :com.time-tracker/enable-worker
                         biff/use-chime)
                       (biff/use-when
                         :com.time-tracker/enable-hawk
                         biff/use-hawk)]})
