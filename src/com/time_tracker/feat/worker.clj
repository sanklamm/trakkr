(ns com.time-tracker.feat.worker
...
(defn print-usage [{:keys [biff/db]}]
  ...
  (let [n-users (nth (q db
    ...
    (log/info "There are" n-users "users."
              ...
              "by setting `:com.time-tracker/enable-worker false` in config.edn)")))

(defn alert-new-user [{:keys [com.time-tracker/enable-worker biff.xtdb/node]} tx]
  (doseq [_ [nil]
          :when enable-worker
